...
#     # 初始化模型并移动到设备
#     ff = FreqFusion(256,256).to(device)
#     # 创建随机输入并移动到设备
#     hr_feat = torch.rand(1, 256, 16, 16).to(device)
#     lr_feat = torch.rand(1, 256, 8, 8).to(device)
#     # 前向传播
...
def hamming2D(M, N):
    ...
    # hamming_x = np.blackman(M)
    # hamming_x = np.kaiser(M)
    hamming_x = torch.hamming_window(M, dtype=torch.float32)
    hamming_y = torch.hamming_window(N, dtype=torch.float32)
    return torch.outer(hamming_x,hamming_y)
...
class FreqFusion(nn.Module):
    def __init__(self,
        ...
        #     self.register_buffer('hamming_highpass', torch.FloatTensor([1.0]))
        # 定义可学习的Hamming窗
        self.hamming_lowpass = nn.Parameter(torch.ones(lowpass_kernel, lowpass_kernel))
        self.hamming_highpass = nn.Parameter(torch.ones(highpass_kernel, highpass_kernel))
        self.init_weights()
    ...
    def init_weights(self):
        ...
        # 可学习 Hamming 窗初始化为 0.8~1.0 之间的值
        with torch.no_grad():  # 避免被误更新
            self.hamming_lowpass.uniform_(0.8, 1.0)
            self.hamming_highpass.uniform_(0.8, 1.0)


    def kernel_normalizer(self, mask, kernel, scale_factor=None, hamming=None):
        if scale_factor is not None:
            mask = F.pixel_shuffle(mask, self.scale_factor)
        
        n, mask_c, h, w = mask.shape  # 解析 mask 形状
        mask_channel = mask_c // (kernel ** 2)

        mask = mask.view(n, mask_channel, kernel, kernel, h, w)  # 变形
        mask = F.softmax(mask, dim=2)

        if hamming is not None:
            hamming = torch.sigmoid(hamming)  # 归一化到 0~1

            # **确保 hamming 窗的形状匹配**
            if hamming.shape != (kernel, kernel):  
                hamming = F.interpolate(hamming[None, None], size=(kernel, kernel), mode="bilinear", align_corners=True)[0, 0]

            # **匹配维度**
            hamming = hamming.view(1, 1, kernel, kernel, 1, 1).to(mask.device)  # 扩展 batch 维度和通道维度
            mask = (mask * hamming).div(mask.sum(dim=(-1, -2), keepdim=True))  # 归一化

        return mask.view(n, mask_channel, h, w, -1).permute(0, 1, 4, 2, 3).contiguous().view(n, -1, h, w)
    ...
    def _forward(self, hr_feat, lr_feat):
        ...
        # 归一化可学习 Hamming 窗
        hamming_lowpass = torch.sigmoid(self.hamming_lowpass)  # 确保值在 0~1
        hamming_highpass = torch.sigmoid(self.hamming_highpass)
        ...
        if self.semi_conv:
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    # 高通滤波 mask
                    mask_hr_hr_feat = self.content_encoder2(compressed_hr_feat)
                    mask_hr_init = self.kernel_normalizer(mask_hr_hr_feat, self.highpass_kernel, hamming=hamming_highpass)
                    compressed_hr_feat = compressed_hr_feat + compressed_hr_feat - carafe(compressed_hr_feat, mask_hr_init, self.highpass_kernel, self.up_group, 1)

                    # 低通滤波 mask
                    mask_lr_hr_feat = self.content_encoder(compressed_hr_feat)
                    mask_lr_init = self.kernel_normalizer(mask_lr_hr_feat, self.lowpass_kernel, hamming=hamming_lowpass)
                    ...
                    mask_lr_init = self.kernel_normalizer(mask_lr, self.lowpass_kernel, hamming=hamming_lowpass)
                    mask_hr_lr_feat = F.interpolate(
        ...
        # 应用可学习 Hamming 窗
        mask_lr = self.kernel_normalizer(mask_lr, self.lowpass_kernel, hamming=hamming_lowpass)
        if self.semi_conv:
            lr_feat = carafe(lr_feat, mask_lr, self.lowpass_kernel, self.up_group, 2)
        else:
            lr_feat = resize(
        ...
        if self.use_high_pass:
            mask_hr = self.kernel_normalizer(mask_hr, self.highpass_kernel, hamming=hamming_highpass)
            hr_feat_hf = hr_feat - carafe(hr_feat, mask_hr, self.highpass_kernel, self.up_group, 1)
            if self.hr_residual:
                hr_feat = hr_feat_hf + hr_feat
            else:
