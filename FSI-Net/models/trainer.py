...
from models.losses import cross_entropy,ohem_cross_entropy,ohem_cross_entropy503
...
from mmseg.models.losses.dice_loss import DiceLoss,dice_loss
from models.ohem_pixel_sampler import OHEMPixelSampler
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.lr = args.lr
        original_weights = [2.4, 2.4, 0.9, 0.9, 3.0]
        self.weights_params =  nn.Parameter(torch.log(torch.tensor(original_weights, dtype=torch.float32)))
        # define optimizers
        ...
        #     self.optimizer_G = optim.AdamW([*self.net_G.parameters(), self.weights_params], lr=self.lr,
        ...
        if args.loss == 'ce':
            ...
            self._pxl_loss = ohem_cross_entropy
        elif args.loss == 'ohem503':
            self._pxl_loss = ohem_cross_entropy503
        elif args.loss == 'fl':
    ...
    def _forward_pass(self, batch):
        ...
        # print(type(self.multi_scale_infer))
    ...
    #                 # gt = F.interpolate(gt, size=pred.size(2), mode="nearest")
    #                 # pred = F.softmax(pred, dim=1) 
    #                 # pred = pred[:, 1:2, :, :] 
    #                 # temp_loss = temp_loss + self.weights[i]*self._pxl_loss(pred, gt, weight=None)
    #                 pass
    #                 # print(self._pxl_loss(pred, F.interpolate(gt, size=pred.size(2), mode="nearest")))
    #             else:
    #                 gt = F.interpolate(gt, size=pred.size(2), mode="nearest")
    #                 # pred_1 = torch.zeros_like(gt)
    #                 # pred = pred_1.scatter_(1, pred.long(), 1)
    #                 pred = F.softmax(pred, dim=1) 
    #                 pred = pred[:, 1:2, :, :] 
    #                 temp_loss = temp_loss + self.weights[i]*self._pxl_loss(pred, gt,weight=None)
    #                 # print(self._pxl_loss(pred, gt))
    #             i+=1
    ...
    def _backward_G(self):
        gt = self.batch['L'].to(self.device).float()
        ohem_sampler = OHEMPixelSampler(context=self.net_G, thresh=0.7, min_kept=25000, ignore_index=255)
        if self.multi_scale_train:
            i         = 0
            temp_loss = 0.0
            for pred in self.G_pred:
                if pred.size(2) != gt.size(2):
                    ...
                    seg_weight = ohem_sampler.sample(pred, resized_gt)
                    temp_loss = temp_loss + self.weights[i] * self._pxl_loss(pred, resized_gt, weight=seg_weight)
                else:
                    #temp_loss = temp_loss + self.weights[i]*self._pxl_loss(pred, gt)
                    seg_weight = ohem_sampler.sample(pred, gt)
                    temp_loss = temp_loss + self.weights[i] * self._pxl_loss(pred, gt, weight=seg_weight)
                i+=1
            self.G_loss = temp_loss
        else:
            seg_weight = ohem_sampler.sample(self.G_pred[-1], gt)
            self.G_loss = self._pxl_loss(self.G_pred[-1], gt,weight=seg_weight)
            #self.G_loss = self._pxl_loss(self.G_pred[-1], gt)
