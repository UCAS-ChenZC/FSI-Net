...
class DecoderTransformer_v3(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
       ...
       # Stage 0: x1 scale
        ...
        _c0   = self.diff_c0(torch.cat((_c0_1, _c0_2), dim=1)) + F.interpolate(_c1, scale_factor=2, mode="bilinear")
...
class ChangeFormerV5(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.TDec_x2   = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False, 
                    ...
                    decoder_softmax = decoder_softmax, feature_strides=[1,2, 4, 8, 16])
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.attn_drop = 0.1
        self.drop_path_rate = 0.1
        #self.FADConv = FADConv(in_channels=3,out_channels=3,kernel_size=3,stride=1) 
...
class SwinChangeFormer(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256,ratio=0.5):
        ...
        #Transformer Decoder
        self.TDec_x2   = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3,4], align_corners=False, 
                    in_channels = self.embed_dims, embedding_dim= self.embedding_dim, output_nc=output_nc, 
                    decoder_softmax = decoder_softmax, feature_strides=[1,2, 4, 8, 16])
    ...
    def forward(self, x1, x2):
        ...
        x1_temp = x1
        x1_temp = self.Maxpool(x1_temp)
        x1_temp = self.Conv2_1(x1_temp)             #输出：(B, , , )
        x2_temp = x2
        x2_temp = self.Maxpool(x2_temp)
        x2_temp = self.Conv2_1(x2_temp)
...
class DAFM(nn.Module):
    ...
    def __init__(self, channel, scaling=16, kernel_size=7):
        ...
        #2024.0111
        self.GlobalAttentionRepViTBlock_1 = GlobalAttentionRepViTBlock(channel,channel,kernel_size=3, stride=1)
        self.GlobalAttentionRepViTBlock_2 = GlobalAttentionRepViTBlock(channel *2 ,channel * 2,kernel_size=3, stride=1)
        self.GlobalAttentionRepViTBlock_3 = GlobalAttentionRepViTBlock(channel * 2,channel,kernel_size=3, stride=1)


    def forward(self,x):
        x1 = x
        x1 = x1 * self.channelattention(x1)
        x1 = x1 * self.GlobalAttentionRepViTBlock_1(x1)
        x1 = x1 * self.spatialattention(x1)
        x1 = torch.concat([x1,x],dim=1)
        w1 = self.Conv1(x1)
        x1 = x1 * w1
        x1 = self.GlobalAttentionRepViTBlock_2(x1)
        x1 = self.GlobalAttentionRepViTBlock_3(x1)
        #x1 = self.conv1_2(x1)
        x  = x + x1 
        return x
