...
def cross_entropy(input, target, weight=None, reduction='mean',ignore_index=255):
    ...
    """
    # print(target.dtype)  # 应该是 torch.int64（long 类型）
    # print(target.min(), target.max())  # 应该在 [0, num_classes - 1] 范围内
...
def ohem_cross_entropy(input, target, weight = None, reduction='mean',ignore_index=255):
    """
    logSoftmax_with_loss
    :param input: torch.Tensor, N*C*H*W
    :param target: torch.Tensor, N*1*H*W,/ N*H*W
    ...
    #weight = None
    loss = F.cross_entropy(input=input, target=target, weight=None,
                           ignore_index=ignore_index, reduction=reduction)
...
def ohem_cross_entropy503(input, target, weight=None, reduction='mean', ignore_index=255):
    ...
    if input.shape[-2:] != target.shape[-2:]:
        input = F.interpolate(input, size=target.shape[1:], mode='bilinear', align_corners=True)
    # 逐像素损失
    loss = F.cross_entropy(input=input, target=target, weight=None, ignore_index=ignore_index, reduction='none')
    # 应用 OHEM 采样的权重
    if weight is not None:
        loss = loss * weight	# weight shape: (N, H, W)

    if reduction == 'mean':
    	# 仅计算被选中的（非ignore）的像素平均
...
def binary_ce(input, target, weight=None, reduction='mean', ignore_index=255):
    """
    计算二分类的 BCE 损失（与 `cross_entropy` 返回值格式相同）
    
    :param input: torch.Tensor, 预测值 (N, C, H, W) 或 (N, H, W) - **未经过 sigmoid**
    :param target: torch.Tensor, 真实标签 (N, H, W) - **二值 0/1**
    :param weight: torch.Tensor, 每个像素点的权重 (可选)
    :param reduction: str, {"mean", "sum", "none"}，默认 "mean"
    :param ignore_index: int, 需要忽略的标签索引（默认 255）
    
    :return: torch.Tensor, 标量损失值，与 `cross_entropy` 一致
    """
    ...
    # **修正 input 形状**
    if input.shape[1] == 2:  # 可能是 (N, 2, H, W)
        input = input[:, 1, :, :]  # 取第 2 个通道，变成 (N, H, W)
    elif input.shape[1] == 1:
        input = input.squeeze(1)  # (N, 1, H, W) -> (N, H, W)

    # **修正尺寸不匹配**
    if input.shape[-2:] != target.shape[-2:]:
        input = F.interpolate(input.unsqueeze(1), size=target.shape[-2:], mode='bilinear', align_corners=True).squeeze(1)
    
    # **确保 target 形状与 input 一致**
    if target.shape[1] == 1:  # (N, 1, H, W) -> (N, H, W)
        target = target.squeeze(1)
    ...
    if ignore_index is not None:
        mask = (target != ignore_index).float()  # 生成忽略 mask
        loss = loss * mask  # 仅计算非忽略区域的损失
        if mask.sum() > 0:
            if reduction == 'mean':
                return loss.sum() / (mask.sum() + 1e-6)  # 避免除零错误
            elif reduction == 'sum':
                return loss.sum()
