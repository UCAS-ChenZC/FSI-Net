...
# def crop_image(img, img_B, label, crop_x, crop_y, img_size):
#     """
#     Crop the images and label based on the given crop position.
#     """
#     img = img[crop_y:crop_y + img_size, crop_x:crop_x + img_size]
#     img_B = img_B[crop_y:crop_y + img_size, crop_x:crop_x + img_size]
#     label = label[crop_y:crop_y + img_size, crop_x:crop_x + img_size] if label is not None else None
...
#         ) if is_train else CDDataAugmentation(img_size=self.img_size)

#         self.crops_per_image = [
#             (Image.open(get_img_path(self.root_dir, name)).size[0] // self.img_size) *
#             (Image.open(get_img_path(self.root_dir, name)).size[1] // self.img_size)
...
#         image_index = 0
#         crop_index = index
#         while crop_index >= self.crops_per_image[image_index]:
#             crop_index -= self.crops_per_image[image_index]
#             image_index += 1

#         name = self.img_name_list[image_index]
...
#         num_crops_x = img_width // self.img_size

#         crop_x = (crop_index % num_crops_x) * self.img_size
#         crop_y = (crop_index // num_crops_x) * self.img_size

#         img, img_B, _ = crop_image(img, img_B, None, crop_x, crop_y, self.img_size)
#         [img, img_B], _ = self.augm.transform([img, img_B], [], to_tensor=self.to_tensor)

#         return {'A': img, 'B': img_B, 'name': name}

#     def __len__(self):
#         return sum(self.crops_per_image)
...
#         img, img_B, label = crop_image(img, img_B, label, crop_x, crop_y, self.img_size)
...
#         return {'A': img, 'B': img_B, 'L': label, 'name': name}
